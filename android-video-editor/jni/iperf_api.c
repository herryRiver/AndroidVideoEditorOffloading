...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    char ch = '\0';
    extern int optind; // reset getopt_long
...
iperf_get_results(struct iperf_test *test)
{
    ...
    test->total_sent = total_sent;
    test->total_recv = total_received;
...
iperf_print_results (struct iperf_test *test)
{
    ...
    SLIST_FOREACH(sp, &test->streams, streams) {
        ...
        if (bytes_sent > 0) {
            ...
            if (test->protocol->id == Ptcp) {
                ...
                LOGI("      Sent\n");
                LOGI(report_bw_format, sp->socket, start_time, end_time, ubuf, nbuf);
            } else {
    ...
    if (test->num_streams > 1) {
        ...
        if (test->protocol->id == Ptcp) {
            ...
            printf(report_sum_bw_format, start_time, end_time, ubuf, nbuf);
            LOGI("      Total sent\n");
            LOGI(report_sum_bw_format, start_time, end_time, ubuf, nbuf);
    ...
    test->total_sent = total_sent;
    test->total_recv = total_received;
    test->total_time = end_time;
    
    LOGI("sent: %llu, recv: %llu, time: %4.2f\n", total_sent, total_received, end_time - start_time);
