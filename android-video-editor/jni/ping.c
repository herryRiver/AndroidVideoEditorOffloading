...
{
    int argc = 2;
    char *argv[] = {"ping", "africandaisy.cc.gt.atl.ga.us"};
    int sock;
     ...
     * disabled for now, as we use SOCK_DGRAM */
    if (0 && getuid() != 0)
    {
    ...
    if((dst_hp = gethostbyname(argv[1])) == NULL)
    {
        if((ip->ip_dst.s_addr = inet_addr(argv[1])) == -1)
        {
            ...
            LOGE("%s: Can't resolve, unknown destination.\n", argv[1]);
            exit(EXIT_FAILURE);
        }
    }
    else
    ...
    LOGI("Source IP: '%s' -- Destination IP: '%s'\n", src_ip, dst_ip);
    ...
    if(setsockopt(sock, IPPROTO_IP, IP_HDRINCL, &on, sizeof(on)) < 0)
    {
        ...
        LOGE("setsockopt() for IP_HDRINCL error");
        exit(EXIT_FAILURE);
    }
    ...
    for(i = 1; i <= num; i++)
    {
        ...
        {
            LOGI("Sent %d byte packet... ", bytes_sent);
            ...
            else if (result > 0)
            {
                ...
                if((bytes_recv = recvfrom(sock, recv_buf, sizeof(ip) + sizeof(icmp) + sizeof(recv_buf), 0, (struct sockaddr *)&dst, (socklen_t *)&dst_addr_len)) < 0)
                {
                    ...
                    long local_rtt = (end.tv_sec - start.tv_sec) * 1000 + (end.tv_nsec - start.tv_nsec) / 1000000;
                    LOGI("Received %d byte packet in %dms\n", bytes_recv, local_rtt);
                    rtt += local_rtt;
                }
            }
            else
