...
iperf_error(char *estr)
{
    ...
    switch (i_errno) {
        case IESERVCLIENT:
            LOGI( "iperf cannot be both server and client\n");
            break;
        case IENOROLE:
            LOGI( "iperf instance must either be a client (-c) or server (-s)\n");
            break;
        case IECLIENTONLY:
            LOGI( "some option you are trying to set is client only\n");
            break;
        case IEDURATION:
            LOGI( "test duration too long (maximum = %d seconds)\n", MAX_TIME);
            break;
        case IENUMSTREAMS:
            LOGI( "number of parallel streams too large (maximum = %d)\n", MAX_STREAMS);
            break;
        case IEBLOCKSIZE:
            LOGI( "block size too large (maximum = %d bytes)\n", MAX_BLOCKSIZE);
            break;
        case IEBUFSIZE:
            LOGI( "socket buffer size too large (maximum = %d bytes)\n", MAX_TCP_BUFFER);
            break;
        case IEINTERVAL:
            LOGI( "report interval too large (maximum = %d seconds)\n", MAX_INTERVAL);
            break;
        case IEMSS:
            LOGI( "TCP MSS too large (maximum = %d bytes)\n", MAX_MSS);
            break;
        case IECTRLWRITE:
            if (errno)
                LOGI( "unable to write to the control socket: %s\n", strerror(errno));
            else
                LOGI( "unable to write to the control socket\n");
            break;
        case IECTRLREAD:
            if (errno)
                LOGI( "unable to read from the control socket: %s\n", strerror(errno));
            else
                LOGI( "unable to read from the control socket\n");
            break;
        case IECTRLCLOSE:
            LOGI( "control socket has closed unexpectedly\n");
            break;
        case IESTREAMWRITE:
            if (errno)
                LOGI( "unable to write to stream socket: %s\n", strerror(errno));
            else
                LOGI( "unable to write to stream socket\n");
            break;
        case IESTREAMREAD:
            if (errno)
                LOGI( "unable to read from stream socket: %s\n", strerror(errno));
            else
                LOGI( "unable to read from stream socket\n");
            break;
        case IESTREAMCLOSE:
            LOGI( "stream socket has closed unexpectedly\n");
            break;
        case IENEWTEST:
            if (errno)
                LOGI( "unable to create a new test: %s\n", strerror(errno));
            else
                LOGI( "unable to create a new test\n");
            break;
        case IECONNECT:
            if (errno)
                LOGI( "unable to connect to server: %s\n", strerror(errno));
            else if (h_errno)
                LOGI( "unable to connect to server: %s\n", hstrerror(h_errno));
            else
                LOGI( "unable to connect to server\n");
            break;
        case IESELECT:
            if (errno)
                LOGI( "select failed: %s\n", strerror(errno));
            else
                LOGI( "select failed\n");
            break;
        case IESENDPARAMS:
            if (errno)
                LOGI( "unable to send parameters to server: %s\n", strerror(errno));
            else
                LOGI( "unable to send parameters to server\n");
            break;
        case IERECVPARAMS:
            if (errno)
                LOGI( "unable to receive parameters from client: %s\n", strerror(errno));
            else
                LOGI( "unable to receive parameters from client\n");
            break;
        case IECREATESTREAM:
            if (errno)
                LOGI( "unable to create a new stream: %s\n", strerror(errno));
            else if (h_errno)
                LOGI( "unable to create a new stream: %s\n", hstrerror(h_errno));
            else
                LOGI( "unable to create a new stream\n");
            break;
        case IEINITSTREAM:
            if (errno)
                LOGI( "unable to initialize stream: %s\n", strerror(errno));
            else if (h_errno)
                LOGI( "unable to initialize stream: %s\n", hstrerror(h_errno));
            else
                LOGI( "unable to initialize stream\n");
            break;
        case IESETWINDOWSIZE:
            if (errno)
                LOGI( "unable to set socket window size: %s\n", strerror(errno));
            else
                LOGI( "unable to set socket window size\n");
            break;
        case IEPACKAGERESULTS:
            if (errno)
                LOGI( "unable to package results: %s\n", strerror(errno));
            else
                LOGI( "unable to package results\n");
            break;
        case IESENDRESULTS:
            if (errno)
                LOGI( "unable to send results: %s\n", strerror(errno));
            else
                LOGI( "unable to send results\n");
            break;
        case IERECVRESULTS:
            if (errno)
                LOGI( "unable to receive results: %s\n", strerror(errno));
            else
                LOGI( "unable to receive results\n");
            break;
        case IESTREAMID:
            LOGI( "stream has an invalid id\n");
            break;
        case IESERVERTERM:
            LOGI( "the server has terminated\n");
            break;
        case IEACCESSDENIED:
            LOGI( "the server is busy running a test. try again later.\n");
            break;
        case IEMESSAGE:
            LOGI( "received an unknown control message\n");
            break;
        case IESENDMESSAGE:
            if (errno)
                LOGI( "unable to send control message: %s\n", strerror(errno));
            else
                LOGI( "unable to send control message\n");
            break;
        case IERECVMESSAGE:
            if (errno)
                LOGI( "unable to receive control message: %s\n", strerror(errno));
            else
                LOGI( "unable to receive control message\n");
            break;
        case IESENDCOOKIE:
            if (errno)
                LOGI( "unable to send cookie to server: %s\n", strerror(errno));
            else
                LOGI( "unable to send cookie to server\n");
            break;
        case IERECVCOOKIE:
            if (errno)
                LOGI( "unable to receive cookie to server: %s\n", strerror(errno));
            else
                LOGI( "unable to receive cookie to server\n");
            break;
        case IELISTEN:
            if (errno)
                LOGI( "unable to start listener for connections: %s\n", strerror(errno));
            else
                LOGI( "unable to start listener for connections\n");
            break;
        case IEACCEPT:
            if (errno)
                LOGI( "unable to accept connection from client: %s\n", strerror(errno));
            else if (h_errno)
                LOGI( "unable to accept connection from client: %s\n", hstrerror(h_errno));
            else
                LOGI( "unable to accept connection from client\n");
            break;
        case IESETNODELAY:
            if (errno)
                LOGI( "unable to set TCP NODELAY: %s\n", strerror(errno));
            else
                LOGI( "unable to set TCP NODELAY\n");
            break;
        case IESETMSS:
            if (errno)
                LOGI( "unable to set TCP MSS: %s\n", strerror(errno));
            else
                LOGI( "unable to set TCP MSS\n");
            break;
        case IEREUSEADDR:
            if (errno)
                LOGI( "unable to reuse address on socket: %s\n", strerror(errno));
            else
                LOGI( "unable to reuse address on socket\n");
            break;
        case IESTREAMCONNECT:
            if (errno)
                LOGI( "unable to connect stream: %s\n", strerror(errno));
            else if (h_errno)
                LOGI( "unable to connect stream: %s\n", hstrerror(h_errno));
            else
                LOGI( "unable to connect stream\n");
            break;
        case IESTREAMACCEPT:
            if (errno)
                LOGI( "unable to accept stream connection: %s\n", strerror(errno));
            else
                LOGI( "unable to accept stream connection\n");
            break;
        case IENONBLOCKING:
            if (errno)
                LOGI( "unable to set socket to non-blocking: %s\n", strerror(errno));
            else
                LOGI( "unable to set socket to non-blocking\n");
            break;
        case IEUPDATETIMER:
            if (errno)
                LOGI( "unable to update timer: %s\n", strerror(errno));
            else
                LOGI( "unable to update timer\n");
            break;
        case IENEWTIMER:
            if (errno)
                LOGI( "unable to create new timer: %s\n", strerror(errno));
            else
                LOGI( "unable to create new timer\n");
            break;
        case IEINITTEST:
            if (errno)
                LOGI( "test initialization failed: %s\n", strerror(errno));
            else
                LOGI( "test initialization failed\n");
            break;
    }
}
